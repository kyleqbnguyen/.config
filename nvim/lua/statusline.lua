...
function StatusLine()
  ...
  local git_parts = {}
  if git.added > 0 then table.insert(git_parts, "+" .. git.added) end
  if git.removed > 0 then table.insert(git_parts, "-" .. git.removed) end
  if git.changed > 0 then table.insert(git_parts, "~" .. git.changed) end
  if #git_parts > 0 then
  ...
  local diag_parts = {}
  if diag.errors > 0 then table.insert(diag_parts, "E:" .. diag.errors) end
  if diag.warns > 0 then table.insert(diag_parts, "W:" .. diag.warns) end
  if diag.info > 0 then table.insert(diag_parts, "I:" .. diag.info) end
  if diag.hints > 0 then table.insert(diag_parts, "H:" .. diag.hints) end
